    ...
    try {
      ...
      if (drops.length) {
        ...
        return self.registration.showNotification('Quicksell — ribasso', {
          ...
          icon: '/icon.svg', tag: 'due', data: { url: drops.length === 1 ? d.vinted_url + '/edit' : '/' },
        });
      }
      ...
      if (n) return self.registration.showNotification('Quicksell', {
        ...
        icon: '/icon.svg', tag: 'pending', data: { url: '/' },
      });
    } catch {}
    return self.registration.showNotification('Quicksell', { body: 'Hai qualcosa da guardare', icon: '/icon.svg', data: { url: '/' } });
  })());
});
